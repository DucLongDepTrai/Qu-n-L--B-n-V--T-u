...
$routeFilter = trim($_GET['tuyen'] ?? '');
$dateFilter = trim($_GET['ngay'] ?? '');
...
$routes = $conn->query("SELECT id, ten_tuyen FROM tuyen_duong ORDER BY ten_tuyen")->fetchAll(PDO::FETCH_ASSOC);
$scheduleSql = "SELECT lt.id, lt.ma_lich_trinh, lt.ngay_di, lt.ngay_den, t.ma_tau, td.ten_tuyen
    FROM lich_trinh lt JOIN tau t ON t.id = lt.id_tau JOIN tuyen_duong td ON td.id = lt.id_tuyen_duong
    WHERE lt.ngay_di > NOW()";
$scheduleParams = [];
if ($routeFilter !== '') {
    $scheduleSql .= ' AND td.id = ?';
    $scheduleParams[] = (int)$routeFilter;
}
if ($dateFilter !== '') {
    $scheduleSql .= ' AND DATE(lt.ngay_di) = ?';
    $scheduleParams[] = $dateFilter;
}
$scheduleSql .= ' ORDER BY lt.ngay_di';
...
.booking{max-width:1100px;margin:30px auto 60px}.booking h1{color:#173f3a;font-size:34px;letter-spacing:-.3px}.booking h2{color:#173f3a}.booking .panel{background:#fff;padding:28px;border:1px solid #e1ebe8;border-radius:14px;box-shadow:0 10px 30px rgba(23,63,58,.07);margin-bottom:24px}.booking .panel > p{color:#60716e;line-height:1.6}.schedule-filter{display:flex;align-items:end;gap:12px;flex-wrap:wrap;margin:20px 0 24px;padding:16px;background:#f4f9f6;border:1px solid #d9e8e3;border-radius:10px}.schedule-filter label{display:flex;flex:1 1 220px;flex-direction:column;gap:7px;color:#34495e;font-size:14px;font-weight:700}.schedule-filter select,.schedule-filter input{padding:10px 11px;border:1px solid #cbdcd6;border-radius:6px;background:#fff;color:#34495e;font:inherit}.clear-filter{padding:10px 4px;color:#1f7a65;font-weight:700;text-decoration:none}.schedule-list{display:grid;gap:14px}.schedule{display:flex;justify-content:space-between;align-items:center;padding:20px;border:1px solid #d9e8e3;border-radius:12px;gap:24px;background:linear-gradient(115deg,#fff,#f7fbf9);transition:.2s}.schedule:hover{border-color:#1f7a65;box-shadow:0 8px 20px rgba(31,122,101,.1);transform:translateY(-2px)}.schedule strong{color:#1f7a65;font-size:16px}.schedule-route{display:flex;gap:10px;align-items:center;margin:10px 0 5px;font-weight:700;color:#173f3a}.schedule-route span{color:#9aada8}.schedule-meta{color:#60716e;font-size:14px}.button{display:inline-block;padding:11px 18px;background:#1f7a65;color:#fff;text-decoration:none;border:0;border-radius:7px;cursor:pointer;font-weight:700;transition:.2s}.button:hover{background:#165a4b;transform:translateY(-1px)}.booking .legend{display:flex;flex-wrap:wrap;gap:16px;margin:18px 0 4px;color:#60716e;font-size:13px}.legend-item{display:flex;align-items:center;gap:7px}.legend-dot{width:12px;height:12px;border-radius:3px;border:1px solid #1f7a65;background:#fff}.legend-dot.selected{background:#1f7a65}.legend-dot.busy{background:#e9ecef;border-color:#adb5bd}.seat{padding:14px 8px;text-align:center;border:1px solid #1f7a65;border-radius:9px;background:#fff;cursor:pointer;min-height:78px;transition:.15s}.seat:hover:not(.busy){background:#eaf4f0;transform:translateY(-2px)}.seat input{display:none}.seat:has(input:checked){background:#1f7a65;color:#fff;box-shadow:0 5px 12px rgba(31,122,101,.25)}.seat.busy{background:#e9ecef;border-color:#adb5bd;color:#6c757d;cursor:not-allowed}.booking .selection-bar{display:flex;justify-content:space-between;align-items:center;gap:16px;margin-top:24px;padding:16px 18px
...
<nav class="customer-nav" style="display:flex;flex-wrap:wrap;gap:10px;margin:0 0 20px;padding:12px 16px;background:#fff;border:1px solid #e1ebe8;border-radius:10px;box-shadow:0 4px 14px rgba(23,63,58,.05)"><a href="<?php echo BASE_URL; ?>">Trang chủ</a><a href="<?php echo BASE_URL; ?>modules/dat-ve/">Đặt vé</a><a href="<?php echo BASE_URL; ?>modules/dat-ve/ve-cua-toi.php">Vé của tôi</a><a href="<?php echo BASE_URL; ?>modules/auth/thong_tin_khach.php">Tài khoản</a><a href="<?php echo BASE_URL; ?>modules/auth/dang_xuat_khach.php">Đăng xuất</a></nav>
...
<div class="panel"><h2>Chọn lịch trình</h2><p>Điều chỉnh tuyến hoặc ngày đi để tìm chuyến phù hợp nhanh hơn.</p><form method="get" class="schedule-filter"><label>Tuyến đường<select name="tuyen"><option value="">Tất cả tuyến</option><?php foreach ($routes as $route): ?><option value="<?php echo $route['id']; ?>" <?php echo (string)$routeFilter === (string)$route['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($route['ten_tuyen']); ?></option><?php endforeach; ?></select></label><label>Ngày đi<input type="date" name="ngay" min="<?php echo date('Y-m-d'); ?>" value="<?php echo htmlspecialchars($dateFilter); ?>"></label><button class="button" type="submit">Tìm chuyến</button><?php if ($routeFilter !== '' || $dateFilter !== ''): ?><a class="clear-filter" href="<?php echo BASE_URL; ?>modules/dat-ve/">Xóa lọc</a><?php endif; ?></form><?php if (!$schedules): ?><div class="empty-state">Không tìm thấy chuyến phù hợp. Hãy thử tuyến hoặc ngày khác.</div><?php else: ?><div class="schedule-list"><?php foreach ($schedules as $schedule): ?><div class="schedule"><div><strong><?php echo htmlspecialchars($schedule['ma_lich_trinh']); ?> · <?php echo htmlspecialchars($schedule['ma_tau']); ?></strong><div class="schedule-route"><?php echo htmlspecialchars($schedule['ten_tuyen']); ?> <span>→</span> <?php echo date('d/m/Y', strtotime($schedule['ngay_di'])); ?></div><div class="schedule-meta">Khởi hành <?php echo date('H:i', strtotime($schedule['ngay_di'])); ?> · Dự kiến đến <?php echo date('d/m/Y H:i', strtotime($schedule['ngay_den'])); ?></div></div><a class="button" href="?lich_trinh=<?php echo $schedule['id']; ?>">Xem ghế trống</a></div><?php endforeach; ?></div><?php endif; ?></div>
